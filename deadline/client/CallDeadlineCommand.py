...
def GetDeadlineCommand():
    ...
    try:
        ...
        #if the error is a key error it means that DEADLINE_PATH is not set. however Deadline 
        # command may be in the PATH or on OSX it could be in the file /Users/Shared/Thinkbox/DEADLINE_PATH
        pass
...
def CallDeadlineCommand( arguments, hideWindow=True, readStdout=True ):
    ...
    if os.name == 'nt':
        if hideWindow:
            # Python 2.6 has subprocess.STARTF_USESHOWWINDOW, and
            # Python 2.7 has subprocess._subprocess.STARTF_USESHOWWINDOW, so check for both.
